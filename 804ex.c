 ...
 * `int fseek(FILE *fp, long offset, int origin)` is identical to lseek except that fp is a file pointer
 * instead of a file descriptor and the return value is an int status, not a position. Write fseek.
		...
		if(f->flag.write)
			...
			if(from == SEEK_CUR) {
				if(offset <= f->count) {
					...
					fp->ptr += offset;
					return 0;
				} else offset -= fp->count;
			}
			fp->count = 0;
			fp->ptr = fp->buff;
		}
	}
	if((pos = lseek(fp->fd,offset,from)) != EOF)
		fp->flag.eof = 0;
	return pos;
}
int _flushbuf(int c, MFILE *fp)
